...
namespace WarehouseManagementSystem.Service
{
    ...
    public class RcsTaskService : IRcsTaskService
    {
        ...
        private readonly IAutoDestinationAllocator _autoDestinationAllocator;
            ...
            IAutoDestinationAllocator autoDestinationAllocator)
        {
            _db = db;
            _logger = logger;
            _autoDestinationAllocator = autoDestinationAllocator;
        }
        ...
        public async Task<TaskResponse> AddTaskAsync(AddTaskRequest request)
        {
            ...
            string lockedLocationNodeRemark = null; // 保存已锁定储位的NodeRemark，用于解锁
            ...
            try
            {
                using (var connection = _db.CreateConnection())
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (request.IsDestinationPreAllocated)
                            {
                                targetPosition = request.destBin;
                                lockedLocationNodeRemark = request.destBin;
                                hasAutoAssignedLocation = true;
                                _logger.LogInformation(
                                    "检测到缓存处理器已预分配终点，AddTaskAsync跳过重复自动分配：TaskType={TaskType}，TargetPosition={TargetPosition}",
                                    request.taskType,
                                    targetPosition);
                            }
                            else
                            {
                                ...
                                var allocationResult = await _autoDestinationAllocator.TryAllocateAsync(
                                    request.taskType,
                                    connection,
                                    transaction);
                                ...
                                {
                                    _logger.LogWarning(
                                        "AddTaskAsync自动分配终点失败：TaskType={TaskType}，GroupName={GroupName}，Reason={Reason}",
                                        request.taskType,
                                        allocationResult.GroupName,
                                        allocationResult.Message);
                                    _logger.LogInformation("准备调用CacheTaskAsync缓存任务");
                                    await CacheTaskAsync(request, connection, transaction, allocationResult.Message);
                                ...
                                lockedLocationNodeRemark = allocationResult.TargetPosition;
                                hasAutoAssignedLocation = true;
                                request.destBin = targetPosition;
                                _logger.LogInformation(
                                    "AddTaskAsync自动分配终点成功：TaskType={TaskType}，TargetPosition={TargetPosition}",
                                    request.taskType,
                                    targetPosition);
                            }
                        }
                        ...
                        else if (ShouldSplitTaskType1(request.sourceBin))
                        {
                            ...
                            try
                            {
                                ...
                                if (!splitTasks.Any() && hasAutoAssignedLocation && !string.IsNullOrEmpty(lockedLocationNodeRemark))
                                {
                                    await _autoDestinationAllocator.ReleaseAsync(lockedLocationNodeRemark, connection, transaction);
                                    _logger.LogInformation($"拆分任务被缓存，已解锁之前自动分配的储位: {lockedLocationNodeRemark}");
                        ...
                        {
                            // 如果之前已锁定储位，需要解锁
                            if (hasAutoAssignedLocation && !string.IsNullOrEmpty(lockedLocationNodeRemark))
                            {
                                await _autoDestinationAllocator.ReleaseAsync(lockedLocationNodeRemark, connection, transaction);
                                _logger.LogWarning($"任务类型无效，已解锁自动分配的储位: {lockedLocationNodeRemark}");
                            }
                            _logger.LogWarning("无效的任务类型: {TaskType}，请求已被拦截", request.taskType);
                            transaction.Rollback();
                            response.status = "Fail";
                        ...
                        if (sourceLocation == null)
                        {
                            ...
                            {
                                await _autoDestinationAllocator.ReleaseAsync(lockedLocationNodeRemark, connection, transaction);
                                _logger.LogWarning($"起点不存在，已解锁自动分配的储位: {lockedLocationNodeRemark}");
                            }
                            _logger.LogWarning("任务起点 {SourceBin} 不存在库位配置中，请求已被拦截", request.sourceBin);
                            transaction.Rollback();
                            response.status = "Fail";
                        ...
                        if (sourceLocation.Lock)
                        {
                            ...
                            if (hasAutoAssignedLocation && !string.IsNullOrEmpty(lockedLocationNodeRemark))
                            {
                                // 解锁之前自动分配的储位
                                await _autoDestinationAllocator.ReleaseAsync(lockedLocationNodeRemark, connection, transaction);
                                _logger.LogInformation($"起点被锁定，已解锁之前自动分配的储位: {lockedLocationNodeRemark}");
                            }
                        ...
                        if (destLocation == null)
                        {
                            ...
                            {
                                await _autoDestinationAllocator.ReleaseAsync(lockedLocationNodeRemark, connection, transaction);
                                _logger.LogWarning($"终点不存在，已解锁自动分配的储位: {lockedLocationNodeRemark}");
                            }
                            _logger.LogWarning("任务终点 {TargetPosition} 不存在库位配置中，请求已被拦截", targetPosition);
                            transaction.Rollback();
                            response.status = "Fail";
                            response.failType = 106;
                        ...
                        // 检查终点是否被锁定
                        if (destLocation.Lock && !request.IsDestinationPreAllocated)
                        {
                            // 终点被锁定，缓存任务而不是返回失败
                            // 如果之前自动分配了储位，需要解锁，因为任务没有创建成功
                            if (hasAutoAssignedLocation && !string.IsNullOrEmpty(lockedLocationNodeRemark))
                            {
                                // 解锁之前自动分配的储位
                                await _autoDestinationAllocator.ReleaseAsync(lockedLocationNodeRemark, connection, transaction);
                                _logger.LogInformation($"终点被锁定，已解锁之前自动分配的储位: {lockedLocationNodeRemark}");
                            }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (hasAutoAssignedLocation && !string.IsNullOrEmpty(lockedLocationNodeRemark))
                        {
                            ...
                            {
                                await _autoDestinationAllocator.ReleaseAsync(lockedLocationNodeRemark, connection, transaction);
                                _logger.LogWarning($"发生异常，已解锁自动分配的储位: {lockedLocationNodeRemark}");
                            }
                            catch (Exception unlockEx)
                            {
                                _logger.LogError(unlockEx, $"解锁储位失败: {lockedLocationNodeRemark}");
                            }
                        }
                        // 回滚事务
                        transaction.Rollback();
        ...
        private async Task<List<RCS_UserTasks>> SplitTaskType1Async(AddTaskRequest request, System.Data.IDbConnection connection, System.Data.IDbTransaction transaction)
        {
            ...
            if (!request.IsDestinationPreAllocated &&
                !await TryLockLocationByNodeRemarkAsync(request.destBin, connection, transaction))
            {
                await UnlockLocationsByNodeRemarkAsync(lockedNodeRemarks, connection, transaction);
                await CacheTaskAsync(request, connection, transaction, $"终点 {request.destBin} 已被锁定");
                return new List<RCS_UserTasks>();
        ...
        private async Task<List<RCS_UserTasks>> SplitTaskType2Async(AddTaskRequest request, System.Data.IDbConnection connection, System.Data.IDbTransaction transaction)
        {
            ...
            {
                // 自动终点任务在 AddTaskAsync 入口或缓存处理器中已经完成终点分配。
                // 这里直接沿用 request.destBin，避免拆分第二段重复分配或重复抢锁。
            ...
            if (!needsAutoAssign && !request.IsDestinationPreAllocated)
            {
        ...
        private async Task<bool> TryLockLocationByNodeRemarkAsync(string nodeRemark, System.Data.IDbConnection connection, System.Data.IDbTransaction transaction)
        {
            var affectedRows = await connection.ExecuteAsync(
                "UPDATE RCS_Locations SET [Lock] = 1 WHERE NodeRemark = @NodeRemark AND ([Lock] = 0 OR [Lock] IS NULL)",
