    ...
    public class TaskService : ITaskService
    {
            ...
            string sortDirection = "desc")
        {
            try
            {
                ...
                {
                    query += " AND sourcePosition LIKE @SourcePosition";
                    countQuery += " AND sourcePosition LIKE @SourcePosition";
                    parameters.Add("@SourcePosition", $"%{sourcePosition.Trim()}%");
                }

                if (!string.IsNullOrWhiteSpace(targetPosition))
                {
                    query += " AND targetPosition LIKE @TargetPosition";
                    countQuery += " AND targetPosition LIKE @TargetPosition";
                    parameters.Add("@TargetPosition", $"%{targetPosition.Trim()}%");
                }

                if (!string.IsNullOrWhiteSpace(robotCode))
                {
                    query += " AND robotCode LIKE @RobotCode";
                    countQuery += " AND robotCode LIKE @RobotCode";
                    parameters.Add("@RobotCode", $"%{robotCode.Trim()}%");
                }

                var pendingActiveTopOrder = "CASE " +
                    "WHEN taskStatus < @TaskFinish AND IsCancelled = 0 THEN 0 " +
                    "ELSE 1 " +
                    "END ASC, ";
                parameters.Add("@TaskFinish", (int)TaskStatuEnum.TaskFinish);

                // 添加排序
                if (sortColumn == "priority")
                {
                    // 使用任务优先级进行排序
                    query += " ORDER BY " + pendingActiveTopOrder +
                            "CASE " +
                ...
                else if (sortColumn == "creatTime")
                {
                    query += " ORDER BY " + pendingActiveTopOrder +
                            "creatTime " + sortDirection;
                }
                else
                {
                    query += $" ORDER BY {pendingActiveTopOrder}{sortColumn} {sortDirection}";
                }
            ...
            string sortDirection = "desc")
        {
            try
            {
                ...
                var cacheSortColumnMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
                {
                    ["taskType"] = "TaskType",
                ...
                if (!cacheSortColumnMap.TryGetValue(sortColumn ?? string.Empty, out var actualSortColumn))
                {
                    actualSortColumn = "CreateTime";
                }
                ...
                query += @"
                    ORDER BY
                        CASE WHEN Status = 0 AND ISNULL(Priority, 0) = 10 THEN 0 ELSE 1 END ASC,
                        CASE WHEN Status = 0 THEN 0 ELSE 1 END ASC, " +
                        $"{actualSortColumn} {normalizedSortDirection}, Id DESC " +
                    @"
        ...
        public async Task<(bool success, string message)> CancelCachedTasks(List<int> ids)
        {
            try
            {
                if (ids == null || ids.Count == 0)
                {
                    return (false, "请选择要取消的缓存任务");
                }
                ...
                try
                {
                    ...
                    if (validIds.Count == 0)
                    {
                        return (false, "请选择有效的缓存任务");
                    }

                    _logger.LogInformation("开始批量取消缓存任务，任务IDs={TaskIds}", string.Join(",", validIds));

                    var affectedCount = await conn.ExecuteAsync(@"
                    ...
                    _logger.LogInformation(
                        "批量取消缓存任务完成，目标数量={TargetCount}，实际取消数量={AffectedCount}",
                        validIds.Count,
                        affectedCount);
                    return (true, $"已取消 {affectedCount} 个缓存任务");
                }
                catch (Exception)
                {
                    transaction.Rollback();
            ...
            {
                _logger.LogError(ex, "批量取消缓存任务失败");
                return (false, $"批量取消任务发生错误: {ex.Message}");
            }
        }
