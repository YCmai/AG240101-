...
namespace WarehouseManagementSystem.Controllers
{
    ...
    public class TasksController : Controller
    {
        ...
        public async Task<IActionResult> Index(int page = 1, string dropLocation = "",
            ...
            TaskType? taskType = null, string sortColumn = "creatTime", string sortDirection = "desc",
            int pageSize = 10, string taskFilter = "user")
        {
            try
            {
                ...
                {
                    // 只获取缓存任务
                    var (cachedItems, cachedTotalItems) = await _taskService.GetCachedTasks(page, pageSize, taskType);
                ...
                {
                    // 默认获取普通任务
                    var (items, totalItems) = await _taskService.GetUserTasks(page, pageSize, filterDate, endDate, taskType, sortColumn, sortDirection);
